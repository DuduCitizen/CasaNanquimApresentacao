...
/* ─────────────────────────────────────────────────────────────
   SCROLL LOCK UNIVERSAL (Chrome Android + iOS Safari)

   Técnica: fixar o body no lugar exato do scroll atual.
   - iOS Safari: overflow:hidden no body desloca fixed elements
     → solução: position:fixed + top:-scrollY no body
   - Chrome Android: a mesma técnica funciona corretamente
     desde que width:100% seja mantido para evitar layout shift
   - O will-change:transform no painel (CSS) cria uma GPU layer
     que elimina o gap visual durante a transição da URL bar
───────────────────────────────────────────────────────────── */
let _scrollY = 0;
